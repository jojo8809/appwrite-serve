...
export const testEmailFunctionality = async (): Promise<{ success: boolean; message: string }> => {
  try {
    console.log("Running email functionality test...");
    
    const businessEmail = "info@justlegalsolutions.org";
    const testSubject = "ServeTracker Email Test - " + new Date().toLocaleString();
    const testBody = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h1 style="color: #2c3e50;">ServeTracker Email Test</h1>
          <p>This is a test email from the ServeTracker application.</p>
          <p>If you received this email, your email sending functionality is working correctly!</p>
